...
static NSTimeInterval _DH_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
CGRect DHCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _DH_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = DHCGRectFitWithContentMode(rect, self.size, contentMode);
